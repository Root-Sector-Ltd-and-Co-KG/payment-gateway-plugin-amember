...
test("aMember publication is only repository-dispatched from default-branch controls", () => {
...
test("aMember PR validation is separate and cannot publish", () => {
...
test("exact-source validation succeeds before write-scoped publication", () => {
  assert.match(workflow, /validate:\s*\n[\s\S]*?permissions:\s*\n\s+contents: read/);
  assert.match(workflow, /php tests\/ipn-v2\.test\.php/);
  ...
  assert.match(workflow, /git -C payment-gateway-plugin-amember archive "\$SOURCE_SHA" \| tar -x -C "\$PACKAGE_DIR"/);
  assert.doesNotMatch(workflow, /\(cd "\$PACKAGE_DIR" && php "\$PREPARE_SCRIPT"/);

  const publish = workflow.slice(workflow.indexOf("  publish:"));
  assert.doesNotMatch(publish, /payment-gateway-plugin-amember|prepare-release\.php|git archive/);
